 ...
 * var doubledNumbers = numbers.mapEither(function(n) {
 *   return flyd.Either.Right(n.right*n.right);
 * });
 */
flyd.mapEither = curryN(2, function(f, s) {
 ...
 * });
 */
function boundMapEither(f) { return flyd.mapEither(f, this); }
...
function createStream() {
  ...
  s.either = function() {
    return flyd.Either.isEither(s.val) ? s.val : flyd.Either.Right(s.val);
  }
  s.hasVal = false;
  s.val = undefined;
  s.vals = [];
  ...
  s.isRight = boundIsRight;
  s.mapEither = boundMapEither;
  return s;
}
