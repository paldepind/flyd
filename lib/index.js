 ...
 *
 * __Signature__: Called bound to `Stream a`: `(Stream a -> Stream b) -> Stream b`
 *
 * @name stream.pipe
...
function createStream() {
  ...
  s.map = deprecated(boundMap, 'map');
  s.ap = deprecated(boundAp, 'ap');
  s.of = flyd.stream;

  s[FL.map] = boundMap;
  s[FL.ap] = fantasy_land_ap;
  s[FL.of] = flyd.stream;
  s[FL.chain] = boundChain;
 ...
 * @private
 */
function deprecated(fn, name) {
  return function deprecator() {
    console.warn('flyd: stream.' + name + ' has been deprecated in favour of using pipe');
